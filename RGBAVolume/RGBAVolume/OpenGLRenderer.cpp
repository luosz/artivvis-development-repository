...
void OpenGLRenderer::UpdateTexture(int currentTimestep, VolumeDataset &volume)
{
	...
	if (volume.elementType == "MET_UCHAR")
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, volume.xRes, volume.yRes, volume.zRes, 0,  GL_RGBA, GL_UNSIGNED_BYTE, volume.memblock3D + (textureSize * currentTimestep));

	else if (volume.elementType == "SHORT")
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA16F, volume.xRes, volume.yRes, volume.zRes, 0, GL_RGBA, GL_UNSIGNED_SHORT, volume.memblock3D + (textureSize * currentTimestep));

	else if (volume.elementType == "FLOAT")
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, volume.xRes, volume.yRes, volume.zRes, 0, GL_RGBA, GL_FLOAT, volume.memblock3D + (textureSize * currentTimestep));
...
GLuint OpenGLRenderer::GenerateTexture(VolumeDataset &volume)
{
	...
	if (volume.elementType == "MET_UCHAR")
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, volume.xRes, volume.yRes, volume.zRes, 0,  GL_RGBA, GL_UNSIGNED_BYTE, volume.memblock3D);

	else if (volume.elementType == "SHORT")
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA16F, volume.xRes, volume.yRes, volume.zRes, 0, GL_RGBA, GL_UNSIGNED_SHORT, volume.memblock3D);

	else if (volume.elementType == "FLOAT")
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, volume.xRes, volume.yRes, volume.zRes, 0, GL_RGBA, GL_FLOAT, volume.memblock3D);
