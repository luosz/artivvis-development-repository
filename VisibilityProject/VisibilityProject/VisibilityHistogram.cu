...
//		dist += sliceGap;
		...
		float yExtent = dist * glm::tan((camera.FoV / 2.0f) * (glm::pi<float>()/180.0f));
		...
		glm::vec3 topLeft = centre + (yExtent * upVec) - (xExtent * rightVec);
		glm::vec3 topRight = centre + (yExtent * upVec) + (xExtent * rightVec);
		glm::vec3 bottomLeft = centre - (yExtent * upVec) - (xExtent * rightVec);
		glm::vec3 bottomRight = centre - (yExtent * upVec) + (xExtent * rightVec);
		...
		else
		{
			...
			CudaGrabFrustum <<<(numFrustumPixels + 255) / 256, 256>>> (numFrustumPixels, frustumExtent, mousePosX, yPixels - mousePosY, cudaHistBins, cudaNumInBin);
		}
